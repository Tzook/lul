...
export default class SocketioRouter extends SocketioRouterBase {
    ...
    private listenToEvents(
        router: SocketioRouterBase,
        events: EVENT[],
        listeners: { on: (string, Function) => {} }[],
        ...
        for (let i in events) {
            ...
            for (let j in listeners) {
                listeners[j].on(event.name, (...args) => {
                    ...
                    if (
                        !socket ||
                        (this.fitThrottle(socket, event, defaultThrottle, routerFn) &&
    ...
    private restartServerEvent(app) {
        ...
        app.post(this.ROUTES.RESTART, this.middleware.isBoss.bind(this.middleware), (req, res) => {
