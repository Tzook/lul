...
import { addBonusPerks, modifyBonusPerks, removeBonusPerks } from "../bonusPerks/bonusPerks.services";
...
import { getMapOfMap, getSetOfMap } from "../utils/maps";
import TalentsServices, { getId, getPerkConfig, getRoom, isMob, isSocket } from "./talents.services";
...
export default class TalentsController extends MasterController {
    ...
    public applyHurtPerks(dmg: number, crit: boolean, attacker: HURTER, target: PLAYER, socket: GameSocket) {
        this.tryToApplyPerk(
            talentsConfig.PERKS.STUN_CHANCE,
            talentsConfig.PERKS.STUN_DURATION,
            talentsConfig.PERKS.STUN_RESISTANCE,
            attacker,
            target,
        );
        this.tryToApplyPerk(
            talentsConfig.PERKS.CRIPPLE_CHANCE,
            talentsConfig.PERKS.CRIPPLE_DURATION,
            talentsConfig.PERKS.CRIPPLE_RESISTANCE,
            attacker,
            target,
        );
        this.tryToApplyPerk(
            talentsConfig.PERKS.BLEED_CHANCE,
            talentsConfig.PERKS.BLEED_DURATION,
            talentsConfig.PERKS.BLEED_RESISTANCE,
            attacker,
            target,
            buffInstace => this.triggerBleed(dmg, crit, buffInstace, attacker, target, socket),
        );
        this.tryToApplyPerk(
            talentsConfig.PERKS.FREEZE_CHANCE,
            talentsConfig.PERKS.FREEZE_DURATION,
            talentsConfig.PERKS.FREEZE_RESISTANCE,
            attacker,
            target,
        );
        this.tryToApplyPerk(
            talentsConfig.PERKS.BURN_CHANCE,
            talentsConfig.PERKS.BURN_DURATION,
            talentsConfig.PERKS.BURN_RESISTANCE,
            attacker,
            target,
            buffInstace => this.triggerBurn(dmg, crit, buffInstace, attacker, target, socket),
        );
    ...
    public applySelfBuffPerks(attacker: PLAYER) {
        ...
        for (let [perkChanceName, perkDurationName] of buffPerks) {
            this.tryToApplyPerk(
                ...
                attacker,
                attacker,
                (buffInstace, buffTarget) => this.addBuffBonusPerks(buffTarget, perkChanceName),
                buffTarget => this.removeBuffBonusPerks(buffTarget, perkChanceName),
            );
        }
    }

    public tryToApplyPerk(
        perkName: string,
        perkDurationName: string,
        perkResistanceName: string,
        attacker: HURTER,
        target: PLAYER,
        onPerkActivated = (buffInstace: BUFF_INSTANCE, buffTarget: PLAYER) => {},
        onPerkCleared = (buffTarget: PLAYER) => {},
    ) {
    ...
    protected applyPerkBuff(
        target: PLAYER,
        perkName: string,
        duration: number,
        onPerkActivated: (buffInstace: BUFF_INSTANCE, buffTarget: PLAYER) => void,
    ...
    protected clearBuff(target: PLAYER, buffInstace: BUFF_INSTANCE) {
        return isSocket(target) ? this.clearSocketBuff(target, buffInstace) : this.clearMobBuff(target, buffInstace);
    }
    ...
    public notifyAboutBuffs(socket: GameSocket) {
        ...
        socket.buffs.forEach(perkBuffs =>
            perkBuffs.forEach(buffInstance =>
                this.tellRemainingBuff(socket.broadcast.to(socket.character.room), buffInstance, socket.character._id),
            ),
        );
        ...
        if (roomObject) {
            _.each(roomObject.sockets, (value, socketId: string) => {
                ...
                otherSocket.buffs.forEach(perkBuffs =>
                    perkBuffs.forEach(buffInstance => this.tellRemainingBuff(socket, buffInstance, socketId)),
                );
            });
        }
    }
    ...
    protected getBuffs(target: PLAYER) {
        return isSocket(target) ? target.buffs : this.getMobBuffsInstance(target.room, target.id);
    }
    ...
    protected triggerBleed(dmg: number, crit: boolean, buffInstance: BUFF_INSTANCE, attacker: HURTER, target: PLAYER, socket: GameSocket) {
        ...
        this.tickDmg(
            ...
            crit,
            buffInstance,
            combatConfig.HIT_CAUSE.BLEED,
            talentsConfig.PERKS_INFO.BLEED_TICK_TIME,
            attacker,
            target,
            socket,
        );
    ...
    protected triggerBurn(dmg: number, crit: boolean, buffInstance: BUFF_INSTANCE, attacker: HURTER, target: PLAYER, socket: GameSocket) {
        ...
        this.tickDmg(
            ...
            crit,
            buffInstance,
            combatConfig.HIT_CAUSE.BURN,
            talentsConfig.PERKS_INFO.BURN_TICK_TIME,
            attacker,
            target,
            socket,
        );
    ...
    public generateTalents(req, res, next) {
        this.services
            .generateTalents(req.body.talents, req.body.perkCollection)
            .then(d => {
    ...
    public warmTalentsInfo(): void {
        this.services.getTalents().catch(e => {
