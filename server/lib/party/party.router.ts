...
import { isInvited, isLeader, isMember, isPartyFull } from "./party.services";
...
export default class PartyRouter extends SocketioRouterBase {
    ...
    [partyConfig.SERVER_INNER.DMG_DEALT.name](
        data: { attacker: HURTER; target: PLAYER; dmg: number; cause: string; crit: boolean },
        socket: GameSocket,
    ) {
        const { target } = data;
        if (isSocket(target)) {
